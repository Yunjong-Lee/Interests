...
- 전송 신호 방식에 따라 Continuous Wave 레이더와 Pulse Radar 로 분류할 수 있으며,  
  정확도가 펄스 방식에 비해 CW 방식이 우수하기 때문에 ADAS용 레이더로는 CW 방식이 주로 사용됩니다.
- 또, 레이더는 검출 거리에 따라 LRR/MRR/SRR 3가지로 구분되며,
  + SRR은 
...
2p] ㅁ ADAS sensor로는 
- 주로 카메라를 사용하며, 보조 수단으로 레이더와 라이다가 있는데, 각각의 센서들의 장단점이 명확하여 최근 통합센서를 개발하는 추세입니다.
- 이들 센서들의 특징으로는 
  + 레이더는 
    - 야간, 악천후 등 날씨 및 원거리 감지 능력이 우수하고, 
    - 거리와 속도, 방향을 감지할 수 있으나
    - 물체의 크기와 형태 등을 인지하기 어렵고, 전파 간섭이 존재하며, 라이다 대비 해상도가 낮다.
  + 카메라는 
    - 사물의 분류가 가능하고 교통신호(color)와 문자 인식이 가능하며, 
  + 반대로 라이다는 
    - 고해상도로 주변을 표현할 수 있으나 
    - 카메라나 레이더 대바 cost 가 매우 높다.
- 레이더차트를 보면, 각각의 센서들의 장단점이 명확함을 알 수 있다.

3p] ㅁ 이와 같이, 레이더는 
- 과거에는 고급차에 장착되어왔으나, 점진적으로 전방은 LRR, 측방과 후방은 MRR/SRR이 장착되고 있습니다.
- 특히 MRR/SRR은 자동차의 주변에 4개 이상 장착될 것으로
